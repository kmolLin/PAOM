...
//static Servo servo[1];
//#define MOVE_SERVO1(I, P) servo[I].move(P)
...
inline void gcode_M31() {
  ...
  /**
   * M928: Start SD Write  -> reset the servo motor
   */
...
inline void gcode_M998(){
    ...
    const int servo_index = parser.value_int();
      if (parser.seen('S'))
        MOVE_SERVO(0, parser.value_int());
      else {
        SERIAL_ECHO_START();
        SERIAL_ECHOPAIR(" Servo ", 0);
        SERIAL_ECHOLNPAIR(": ", servo[0].read());
      }

  //if (HAS_SERVOS){SERIAL_ERROR_START();
  //SERIAL_ECHOPAIR("Servo ", 5);}  
  //MOVE_SERVO1(0, 50);
  }
